...
contract('BondingCurve app', accounts => {
  ...
  const initialize = async _ => {
    ...
    await acl.grantPermission(authorized2, curve.address, CREATE_SELL_ORDER_ROLE, {
      from: root
    })
  ...
  context('> #createBuyOrder', () => {
    context('> sender has CREATE_BUY_ORDER_ROLE', () => {
      context('> and collateral is whitelisted', () => {
        ...
        context('> but value is zero', () => {
          it('it should revert', async () => {
            await assertRevert(() => curve.createBuyOrder(authorized, token1.address, 0, {
              from: authorized,
              value: BUY_GAS
            }))
          })
      ...
      context('> but collateral is not whitelisted', () => {
        it('it should revert', async () => {
          ...
          await assertRevert(() => curve.createBuyOrder(authorized, unlisted.address, 10, {
            from: authorized,
            value: BUY_GAS
          }))
        })
    ...
    context('> sender does not have CREATE_BUY_ORDER_ROLE', () => {
      it('it should revert', async () => {
        await assertRevert(() => curve.createBuyOrder(unauthorized, token3.address, 10, {
          from: unauthorized,
          value: BUY_GAS
        }))
      })
  ...
  async function sellAsMuchAsPossible({
    address,
    collateralToken
  }) {
  ...
  async function buyAndClaimTokens({
    address,
    collateralToken,
    amount,
    from
...
function getSell({
  ...
  let k = amount
  let slope = (m.div((n.add(1)))).mul(s.pow(n.add(1))).mul( ((k.div(s).add(1)).pow(n.add(1))).sub(1) )
  return {
    bancor,
    slope,
    m,
...
function getBuy({
  ...
  let slope = totalSupply.mul(
    (
      amount.mul(
        n.add(1)
      ).div(
        m.mul(
          totalSupply.pow(n.add(1))
        )
      ).add(1)
    ).pow(
      (new Decimal(1)).div(
  ...
  let bancor = totalSupply.mul(
    (
      (new Decimal(1)).plus(amount.div(balance))
    ).pow(reserveRatio).sub(1)
  )
  return {
    bancor,
    slope,
