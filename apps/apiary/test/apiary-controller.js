...
contract('ApiaryController app', accounts => {
  ...
  let ETH, APP_MANAGER_ROLE, POOL_TRANSFER_ROLE, TM_MINT_ROLE, TM_BURN_ROLE, CURVE_CREATE_BUY_ORDER_ROLE, CURVE_CREATE_SELL_ORDER_ROLE
  ...
  const initialize = async _ => {
    ...
    curve = await BancorCurve.at(getEvent(bcReceipt, 'NewAppProxy', 'proxy'))
    // apiary controller
    ...
    // permissions
    await acl.createPermission(curve.address, pool.address, POOL_TRANSFER_ROLE, root, { from: root })
    await acl.createPermission(curve.address, tokenManager.address, TM_MINT_ROLE, root, { from: root })
    await acl.createPermission(curve.address, tokenManager.address, TM_BURN_ROLE, root, { from: root })
    await acl.createPermission(apiary.address, curve.address, CURVE_CREATE_BUY_ORDER_ROLE, root, { from: root })
    await acl.createPermission(apiary.address, curve.address, CURVE_CREATE_SELL_ORDER_ROLE, root, { from: root })
    // collaterals
    ...
    await curve.initialize(apiary.address, tokenManager.address, formula.address, 1)
  }
  ...
  before(async () => {
    ...
    CURVE_CREATE_BUY_ORDER_ROLE = await bcBase.CREATE_BUY_ORDER_ROLE()
    CURVE_CREATE_SELL_ORDER_ROLE = await bcBase.CREATE_SELL_ORDER_ROLE()
  })
