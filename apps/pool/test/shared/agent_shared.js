...
module.exports = (agentName, { accounts, artifacts, web3 }) => {
  const Agent = artifacts.require(agentName)
  ...
  contract(`> Shared tests for Agent-like apps`, accounts => {
      ...
      for (const depositAmount of [0, 3]) {
        context(depositAmount ? '> With ETH' : '> Without ETH', () => {
          ...
          it('can execute expensive fallback actions', async () => {
            ...
            assert.equal(
              (await getBalance(expensiveFallbackTarget.address)).toString(),
              depositAmount,
              'expected ending balance of execution target to be correct'
            )
            assert.equal((await getBalance(agent.address)).toString(), 0, 'expected ending balance of agent at end to be 0')
          })
          ...
          it('can execute with data when target is not a contract', async () => {
            ...
            assertEvent(receipt, 'Execute')
            assert.equal(
              (await getBalance(nonContract)).toString(),
              nonContractBalance.add(depositAmount).toString(),
              'expected ending balance of non-contract to be correct'
            )
            assert.equal((await getBalance(agent.address)).toString(), 0, 'expected ending balance of agent at end to be 0')
          })
          ...
          it('fails to execute without permissions', async () => {
            ...
            await assertRevert(() =>
              agent.execute(executionTarget.address, depositAmount, data, {
                from: nonExecutor,
              })
            )
          })
          ...
          it('fails to execute actions with more ETH than the agent owns', async () => {
            ...
            await assertRevert(() =>
              agent.execute(executionTarget.address, depositAmount + 1, data, {
                from: executor,
              })
            )
          })
    ...
    context('> Signing messages', () => {
      ...
      const assertIsValidSignature = (isValid, erc1271Return) => {
        const expectedReturn = isValid ? ERC1271_RETURN_VALID_SIGNATURE : ERC1271_RETURN_INVALID_SIGNATURE
      ...
      context('> Designated signer', () => {
        ...
        const eip712Sign = async (hash, key) => ({
          ...
          ...ethUtil.ecsign(Buffer.from(hash.slice(2), 'hex'), Buffer.from(key, 'hex')),
        })

        const signFunctionGenerator = (signFunction, signatureModifier) => async (hash, signerOrKey, useLegacySig = false, useInvalidV = false) => {
        ...
        const addERC1271ModePrefix = signature => `${SIGNATURE_MODES.ERC1271}${signature.slice(2)}`

        const createChildAgentGenerator = designatedSigner => async () => {
          const agentReceipt = await dao.newAppInstance(agentAppId, agentBase.address, '0x', false)
          const agentProxyAddress = getEvent(agentReceipt, 'NewAppProxy', 'proxy')
          ...
          await childAgent.setDesignatedSigner(designatedSigner, {
            from: signerDesignator,
          })
        ...
        for (const {
          ...
          context(`> Signature mode: ${name}`, () => {
            beforeEach(async () => {
              ...
              await agent.setDesignatedSigner(signer, {
                from: signerDesignator,
              })
            })
        ...
        context('> Signature mode: ERC1271', () => {
          ...
          const setDesignatedSignerContract = async (...params) => {
            ...
            return agent.setDesignatedSigner(designatedSigner.address, {
              from: signerDesignator,
            })
          }
        ...
        context(`> Signature mode: invalid modes`, () => {
          ...
          beforeEach(async () => {
            await agent.setDesignatedSigner(randomAccount, {
              from: signerDesignator,
            })
          })
