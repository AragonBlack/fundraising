...
const balances = [{ amount: 123.576, symbol: 'ETH' }, { amount: 123.576, symbol: 'DAI' }, { amount: 123.576, symbol: 'ANT' }]
...
class Balances extends React.Component {
  ...
  updateConvertedRates = throttle(async ({ balances }) => {
    const verifiedSymbols = balances.filter(({ verified }) => verified).map(({ symbol }) => symbol)
  ...
  render() {
    ...
    // const balanceItems = balances.map(({ numData: { amount, decimals }, symbol, verified }) => {
    //   const adjustedAmount = amount / Math.pow(10, decimals)
    //   const convertedAmount = verified && convertRates[symbol] ? adjustedAmount / convertRates[symbol] : -1
    //   return {
    //     symbol,
          ...
          <List>
            {balanceItems.length > 0 ? (
              balanceItems.map(({ amount, convertedAmount, symbol }) => (
                <ListItem key={symbol}>
                  <BalanceToken amount={amount} convertedAmount={convertedAmount} symbol={symbol} verified={verified} />
                </ListItem>
              ))
            ) : (
              <EmptyListItem />
