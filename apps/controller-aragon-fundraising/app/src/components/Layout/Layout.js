...
function useLayout(parentWidth) {
  ...
  const [name, width] = useMemo(() => getLayoutSize(parentWidth === undefined ? viewportWidth : parentWidth), [viewportWidth, parentWidth])
...
function Layout({ children, parentWidth }) {
  ...
  return (
    <div>
      <div css="margin: 0 auto" style={{ width: width === 360 ? '100%' : `${width}px` }}>
        {children}
      </div>
    </div>
  )
