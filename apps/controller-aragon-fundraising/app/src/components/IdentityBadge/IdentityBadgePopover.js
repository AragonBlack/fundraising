...
class IdentityBadgePopover extends React.PureComponent {
  ...
  render() {
    const { address, connectedAccount, popoverAction, networkType, onClose, opener, title, visible } = this.props
    ...
    return (
      <Popover visible={visible} opener={opener} onClose={onClose}>
        ...
        >
          ...
          {popoverAction && (
            <Button mode="outline" size="small" onClick={this.handlePopoverActionClick}>
              {popoverAction.label}
            </Button>
          )}
        </section>
