...
module.exports = async function deployTemplate(web3, artifacts, templateName, contractName, apps = APPS) {
  ...
  if (!artifacts)
    errorOut('Missing "artifacts" object. This script must be run with an "artifacts" object globally defined, for example through "truffle exec".')
  ...
  const deployer = new TemplatesDeployer(web3, artifacts, owner, { ens, verbose: true, apps })
