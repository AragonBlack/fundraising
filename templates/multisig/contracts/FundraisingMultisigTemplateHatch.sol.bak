...
contract FundraisingMultisigTemplateHatch is BaseTemplate {
    ...
    bytes32 constant private BANCOR_FORMULA_ID = _apmNamehashHatch("bancor-formula");
    bytes32 constant private MARKET_MAKER_ID = _apmNamehashHatch("batched-bancor-market-maker");
    bytes32 constant private TAP_ID = _apmNamehashHatch("tap");
    bytes32 constant private ARAGON_FUNDRAISING_ID = _apmNamehashHatch("aragon-fundraising");
    ...
    bytes32 constant public HATCH_NODE = keccak256(APM_NODE, keccak256("hatch"));
    ...
    function _apmNamehashHatch(string name) internal pure returns (bytes32) {
        return keccak256(HATCH_NODE, keccak256(name));
    }
}
